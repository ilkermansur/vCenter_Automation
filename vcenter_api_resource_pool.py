...
###
rp_list_url = f"https://{vcenter_ip}/api/vcenter/resource-pool"

response = requests.get(url=rp_list_url, headers={"vmware-api-session-id": session_id}, verify=False)
if response.status_code == 200:
    rp_list = response.json()
    for rp in rp_list:
        resource_pool_id = rp["resource_pool"]
        resource_pool_name = rp["name"]
        print(f"Resource Pool ID: {resource_pool_id}, Resource Pool Name: {resource_pool_name}")
